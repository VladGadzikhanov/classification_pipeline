...
from src.models.efficeintnet.utils import (
...
class MBConvBlock(nn.Module):
    ...
    Args:
        block_args (namedtuple): BlockArgs, defined in general.py.
        global_params (namedtuple): GlobalParam, defined in general.py.
    ...
    def one_shot_predict(self, batch, device):
        ...
        return (
            tags,
            np.c_[np.array(names), np.array(imgs_path), targets.numpy(), batch_preds.numpy(), batch_probs.numpy()],
        )
...
class EfficientNet(nn.Module):
    ...
    def __init__(self, blocks_args=None, global_params=None, use_attention=True):
        super().__init__()
    ...
    def forward(self, inputs):
        ...
        """
        x = inputs.transpose(0, 1)  # BGHW -> GBHW
        # Convolution layers
        x = torch.stack([self.fc(self._avg_pooling(self.extract_features(y)).flatten(start_dim=1)) for y in x])
        x = x.transpose(0, 1)  # GBF -> BGF
        # attention layer:
        if self.use_attention:
            attn_weights = F.softmax(self.attn(x).squeeze(-1), dim=1).unsqueeze(1)
    ...
    @classmethod
    def from_pretrained(
        cls, model_name, weights_path=None, advprop=False, in_channels=3, num_classes=1000, **override_params
    ):
        """Create an efficientnet model according to name.
        Args:
            model_name (str): Name for efficientnet.
...
def EfficientnetB3AttnPretrained(num_classes=1000):
